...
def get_color_list(number: int):
    ...
    color_list = [
        ...
        (0, 0, 128),  # navy
        (238, 130, 238),  # violet
        (85, 107, 47),  # OliveDrab	橄榄土褐色 556B2F

        (255, 20, 147),  # deep pink

    ]

    # append color list with RGB list
    _color_list = []

    if int(number) <= len(color_list):
        for i in range(int(number)):
            _color_list.append(get_carla_color(color_list[i][0], color_list[i][1], color_list[i][2]))
    else:
        for i in range(len(color_list)):
            _color_list.append(get_carla_color(color_list[i][0], color_list[i][1], color_list[i][2]))

        appending_number = int(number) - len(color_list)
        for i in range(appending_number):
            _color_list.append(get_random_color())

    return _color_list


def get_carla_color(r, g, b):
    """
    Get a carla color by setting the rgb value.
    """
...
def get_random_color():
    ...
    carla_color = carla.Color(r=r, g=g, b=b)
...
LightPink = carla.Color(255, 182, 193)
Pink = carla.Color(255, 192, 203)
Crimson = carla.Color(220, 20, 60)
LightBLue = carla.Color(173, 216, 230)
