...
def visualize_actor(carla_debug_helper, vehicle, color=red, thickness=0.5, duration_time=0.5):
    """
    Visualize an vehicle(or walker) in server by drawing its bounding box.
    """
    bbox = vehicle.bounding_box
...
def plot_actor_bbox(actor, thickness=0.5, color=red, duration_time=0.1):
    ...
    # plot center of actor
    bbox_location = bbox.location
    actor_location = actor.get_location()

    bbox.location = bbox_location + actor_location

    debug_helper.draw_box(
        box=bbox,
        rotation=rotation,
        thickness=thickness,
        color=color,
    ...
    # # todo fix draw vehicle center point, hard to see in rollout
    # # plot actor center point
    # bbox_location.z = 5.
