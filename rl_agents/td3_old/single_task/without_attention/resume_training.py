"""
todo merge this script with original rl_agent

This is a developing script for resuming training process of a model.

Major function:
 - Load previous model dict
...
def resume_training(args, rl_config, model_path):

    # restore NN before init env

    configProto = init_tensorflow()

    # Env4fixed is the most recent version of env
    env = CarlaEnv4Fixed(
    ...
    with tf.compat.v1.Session(config=configProto) as sess:
        ...
        # todo check if actor and critic weights are successfully restored
