...
import logging
import pickle
from rl_agents.td3_old.single_task.without_attention.rl_utils import ActorNetwork, CriticNetwork, Memory, get_split_batch
...
# todo use numpy to store training data
training_data = []
...
def train_loop(args, rl_config):
    ...
    # todo if tuning, use CarlaEnv4

    env = CarlaEnv3(attention=False,
                    multi_task=False,
                    route_option='left',
        ...
        for episode in range(1, rl_config.total_episodes+1):
            ...
            while True:
                ...
                if done:
                    ...
                    # f = open(os.path.join(result_path, "reward_data"+".pkl"), 'wb')
                    ...
                    if episode % rl_config.model_save_frequency_high_success == 0:
                        if rl_config.model_save_frequency and avarage_success_rate >= 0.75:
                            print('episode: ', episode, 'success rate: ', avarage_success_rate)
                            try:
                    ...
                    # pickle.dump(d, fw, pickle.HIGHEST_PROTOCOL)
                    # fw.close()
...
def render_loop(args):
    try:
        ...
        # todo fix state dimension according to args
...
def main():
    ...
    argparser.add_argument(
        ...
        action='store_true',
        dest='debug',
        help='state representation of ego vehicle')
    argparser.add_argument(
        '--npc_state',
        action='store_true',
        dest='debug',
