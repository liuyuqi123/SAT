...
from rl_agents.td3_old.single_task.with_attention.rl_config import hyperParameters
...
# ==============================================================================
class ActorNetwork():
    def __init__(self, state_size, action_size, tau, learning_rate, net_name):
...
class CriticNetwork():
    def __init__(self, state_size, action_size, tau, learning_rate, net_name):
        ...
        self.action_size = action_size
        self.optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate)
        self.optimizer_2 = tf.compat.v1.train.AdamOptimizer(learning_rate)
        self.tau = tau
        self.rl_config = hyperParameters()

        self.state_inputs, self.action, self.critic_variables, self.q_value = self.build_critic_network(net_name)
        self.state_inputs_target, self.action_target, self.critic_variables_target, self.q_value_target = self.build_critic_network(net_name + "_target")
    ...
    def train(self, sess, state, action, target, ISWeights):
        _, _, loss, absolute_errors = sess.run([self.optimize, self.optimize_2, self.loss, self.absolute_errors], feed_dict={
            self.state_inputs: state,
            self.action: action,
            self.target: target,
