...
class SocActorNetwork:

    def __init__(self, config, net_name):
        """
        Actor with social attention mechanism.
        """

        self.config = config
        self.state_size = self.config.state_size_with_att_mask
        self.action_size = self.config.action_size

        # use decayed learning rate
        if config.use_lra_decay:
        ...
        # fixed parameters for attention mechanism
        ...
        self.state_inputs, self.actor_variables, self.action, self.att_matrix = self.build_actor_network(net_name)
        self.state_inputs_target, self.actor_variables_target, self.action_target, self.att_matrix_target = \
            self.build_actor_network(net_name + "_target")
    ...
    def split_input(self, state_with_att_mask):
        """
        state:
        [batch, ego_feature_num + npc_feature_num*npc_num + mask]
        """

        vehicle_state = state_with_att_mask[:, 0:self.config.ego_feature_num + self.config.npc_num*self.config.npc_feature_num]  # Dims: batch, (ego+npcs)features
        ...
        vehicle_mask = state_with_att_mask[:, -self.config.vehicle_mask_size:]  # Dims: batch, len(mask)
        vehicle_mask = vehicle_mask < 0.5

        return ego_state, npc_state, vehicle_mask
    ...
    def build_actor_network(self, name):
        with tf.compat.v1.variable_scope(name):
            ...
            ego_state, npc_state, vehicle_mask = self.split_input(state_inputs)
            # vehicle encoders
            ...
            # Dims: batch, npc_entities + 1, 64
            vehicle_encoders_output = tf.concat([ego_encoder_2, npc_encoder_2], axis=1)
    ...
    def get_attention_matrix(self, sess, state):
        """
        todo fix this method, retrieve attention weight
        """
    ...
    def train(self, sess, state, action_gradients):
        ...
        # print('-' * 10, 'Actor, before run train', '-' * 10)
...
class SocCriticNetwork:
    ...
    def __init__(self, config, name):
        ...
        if self.config.use_lrc_decay:
            self.optimize = self.optimizer.minimize(self.loss, global_step=self.global_step_1)
            self.optimize_2 = self.optimizer_2.minimize(self.loss_2, global_step=self.global_step_2)
        else:
        ...
        self.update_target_op = [self.critic_variables_target[i].assign(
            tf.multiply(self.critic_variables[i], self.tau) + tf.multiply(self.critic_variables_target[i], 1 - self.tau)
    ...
    def build_critic_network(self, name):
        with tf.compat.v1.variable_scope(name):
            ...
            ego_state, npcs_state, _ = self.split_input(state_inputs)
            ...
            concat = tf.concat([npc_encoder_2[:, i] for i in range(5)], axis=1, name="concat")

            fc_1 = tf.concat([ego_encoder_2, concat], axis=1, name="fc_1")
    ...
    def train(self, sess, state, action, target, ISWeights):
        ...
        # # debug global step and lr
        # print('-' * 10, 'Critic, before run train', '-' * 10)
        # print('global_step_1: ', sess.run(self.global_step_1))
        # print('lrc_1: ', sess.run(self.learning_rate_1))
        # print('global_step_2: ', sess.run(self.global_step_2))
        # print('lr_2: ', sess.run(self.learning_rate_2))
        # print('')
        ...
        # # debug global step and lr
        # print('-' * 10, 'Critic, After run train', '-' * 10)
        # print('global_step_1: ', sess.run(self.global_step_1))
        # print('lrc_1: ', sess.run(self.learning_rate_1))
        # print('global_step_2: ', sess.run(self.global_step_2))
        # print('lr_2: ', sess.run(self.learning_rate_2))
